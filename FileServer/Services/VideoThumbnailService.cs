...
public class VideoThumbnailService : IVideoThumbnailService, IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _thumbnailsRoot = Path.Combine(rootPath, _config.ThumbnailDirectory);
    ...
    private async Task ScanAndPreGenerateThumbnails()
    {
        try
        {
            ...
            {
                _logger.LogWarning("配置中未设置 VideoLibraryDirectory，跳过视频扫描");
                return;
            }
            ...
            var videoFiles = await Task.Run(() => ScanVideoFiles(videoLibraryPath));
    ...
    private List<string> ScanVideoFiles(string directory)
    {
    ...
    public async Task<VideoThumbnailResponse> GenerateThumbnailAsync(VideoThumbnailRequest request)
    {
        try
        {
            ...
            if (!await _fileService.FileExistsAsync(request.VideoPath))
            {
            ...
            var duration = await GetVideoDurationAsync(request.VideoPath);
            TimeSpan thumbnailTime;
            TimeSpan? videoDuration = duration;

            if (duration != null)
            {
                var positionPercentage = request.PositionPercentage ?? _config.ThumbnailPositionPercentage;
    ...
    {
        return await _fileService.FileExistsAsync(videoPath);
    }
    ...
    public async Task<TimeSpan?> GetVideoDurationAsync(string videoPath)
    {
        try
        {
            ...
            var physicalPath = Path.Combine(_fileService.GetRootPath(), videoPath);
    ...
    public async Task<List<VideoThumbnailResponse>> GenerateThumbnailsBatchAsync(List<VideoThumbnailRequest> requests)
    {
        ...
        var tasks = requests.Select(request => ProcessThumbnailRequestWithSemaphore(request, semaphore)).ToList();
    ...
    private async Task<VideoThumbnailResponse> ProcessThumbnailRequestWithSemaphore(VideoThumbnailRequest request, SemaphoreSlim semaphore)
    {
        ...
        {
            var result = await GenerateThumbnailAsync(request);
            if (result.Success) Interlocked.Increment(ref _generatedCount);
            return result;
        }
        finally
        {
    ...
    private async Task<bool> GenerateThumbnailWithFFmpeg(string videoPath, string thumbnailPath, TimeSpan time, int width, int height)
    {
        try
        {
            ...
            var physicalVideoPath = Path.Combine(_fileService.GetRootPath(), videoPath);
            ...
            await Task.Delay(100);
            if (process.ExitCode == 0 && File.Exists(thumbnailPath)) return true;
            else
            {
                var error = errorBuilder.ToString();
                _logger.LogWarning("FFmpeg缩略图生成失败，退出代码: {ExitCode}", process.ExitCode);
                if (!string.IsNullOrEmpty(error)) _logger.LogWarning("FFmpeg错误详情: {Error}", error);
                return false;
            }
        }
        catch (Exception ex)
    ...
    private string GetThumbnailFilePath(VideoThumbnailRequest request)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(videoDir)) videoDir = "root";
            else videoDir = videoDir.Replace(Path.DirectorySeparatorChar, '_');

            var uniqueKey = $"{videoDir}_{videoFileName}_{request.Width}x{request.Height}";
            using var md5 = System.Security.Cryptography.MD5.Create();
            var hash = BitConverter.ToString(md5.ComputeHash(Encoding.UTF8.GetBytes(uniqueKey))).Replace("-", "").ToLower();
            var format = string.IsNullOrEmpty(request.OutputFormat) ? _config.ThumbnailFormat : request.OutputFormat;
    ...
    private void ValidateFFmpegTools()
    {
        try
        {
            ...
            if (File.Exists(_config.FFmpegPath))
            {
                var psi = new ProcessStartInfo { FileName = _config.FFmpegPath, Arguments = "-version", RedirectStandardOutput = true, UseShellExecute = false, CreateNoWindow = true };
                using var process = Process.Start(psi);
                if (process != null)
                {
                    process.WaitForExit(5000);
                    if (process.ExitCode == 0) _logger.LogInformation("FFmpeg工具验证成功: {FfmpegPath}", _config.FFmpegPath);
                }
            }
            else _logger.LogWarning("FFmpeg工具不存在: {FfmpegPath}", _config.FFmpegPath);

            if (File.Exists(_config.FFprobePath))
            {
                var psi = new ProcessStartInfo { FileName = _config.FFprobePath, Arguments = "-version", RedirectStandardOutput = true, UseShellExecute = false, CreateNoWindow = true };
                using var process = Process.Start(psi);
                if (process != null)
                {
                    process.WaitForExit(5000);
                    if (process.ExitCode == 0) _logger.LogInformation("FFprobe工具验证成功: {FfprobePath}", _config.FFprobePath);
                }
            }
            else _logger.LogWarning("FFprobe工具不存在: {FfprobePath}", _config.FFprobePath);
        }
        catch (Exception ex)
        {
